...
void update(){
  ...
  if(dist(player.pos.x,player.pos.y,companion.pos.x-player.pos.x,companion.pos.y)<=player.dim.x/2+companion.dim.x)player.companion=true;
  player.update();
  }
}
//----------------------------------------------------------------
...
void addWorldThree(){
  ...
  enemies.add(new BasicEnemy(new PVector(300,height-125),new PVector(0,0)));
...
void companionLocation(){
  if (gameState==1){
    ...
    companion=new Companion(new PVector(1800,height-580),new PVector (0,0));
  }
  else if(gameState==3){
